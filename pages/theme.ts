...
export class ThemePage {
  ...
  async theme_list() {
    ...
    expect(theme_list.status()).toBeTruthy();
  ...
  async free_theme_create(theme_name) {
    ...
    const free_theme_create = await this.request.post(`${base_url}/v1/onboarding/basic-information/`, {
      headers: {
        'authorization': auth,
      },
      data: {
  ...
  async pro_theme_create(theme_name) {
    ...
    const pro_theme_create = await this.request.post(`${base_url}/v1/onboarding/basic-information/`, {
      headers: {
        'authorization': auth,
      },
      data: {
  ...
  async theme_update(theme_slug, new_theme_name) {
    ...
    const theme_update = await this.request.put(`${base_url}/v1/themes/${theme_slug}`, {
      headers: {
        'authorization': auth,
      },
      data: {
    ...
    expect(theme_update.status()).toBeTruthy();
